...
class LauncherApp(QMainWindow): 
    ...
    def populate_grid(self, filtered_roms):
        ...
        for index, game_data in enumerate(filtered_roms):
            ...
            # --- ИСПРАВЛЕНИЕ KeyError: 'rom' ---
            if 'rom' not in game_data:
                logger.warning(f"Пропущена игра без ROM-файла: {game_data.get('title', 'Неизвестная игра')}")
                continue # Пропускаем элемент, который не был полностью инициализирован
            # -----------------------------------
            ...
            # 🌟 ИСПРАВЛЕНО: Добавлен недостающий позиционный аргумент item_size
            loader = ImageLoaderThread(
                item_widget, 
                game_data['folder'], 
                item_widget.image_label.size(), # <--- НОВОЕ: Передаем QSize
